...
static char	my_pix_data [1024];	/* my pixmap */
unsigned int 	pix_width, pix_height; 	/* pixmap size */
double		re_center, im_center, cwidth;
int		itermax;
int 		i;
...
(void) mandelbrot_out (data, data_width, data_height,
               re_center, im_center, cwidth, itermax);
#endif

printf ("filled in the circle \n");

initialize_RGB_window (argc, argv, data_width, data_height);

printf (" initialized the window \n");

pix_width = data_width;

for (i=0; i<data_height; i+=2) {

    (void) add_float_data_to_pixmap ((data + data_width*i), data_width, 0, 0, 
			my_pix_data, pix_width, 1);

    (void) add_block_to_window (my_pix_data, pix_width, 1, 0, i);
