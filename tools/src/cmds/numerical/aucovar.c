...
/* ========================================================== */
/* 
 * This program reads the indicated file, computes the auto-covariance
...
#endif
{
   ...
   for (i=3; i<argc; i++) {
      ...
      for (j=0; j<NSPEC; j++) {
         ...
         delta = timeDelta;
         S = AutoCoVariance (ydata, npoints, delta); 
#ifdef SMOOTH
         center = npoints / 2;
         width = npoints / 8;
         S = GAutoCoVariance (ydata, npoints, delta, center, width);
#endif /* SMOOTH */
